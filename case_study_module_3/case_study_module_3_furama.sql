...
(6,'Tôn Nữ Mộc Châu','2005-12-06',0,'732434211','0988888844','tonnuchau@gmail.com','37 Yên Thế, Đà Nẵng',4),
...
(ifnull(dich_vu.chi_phi,0)+sum(ifnull(dich_vu_di_kem.gia,0)*ifnull(hop_dong_chi_tiet.so_luong,0))) as tong_tien
from loai_khach 
join khach_hang
on loai_khach.ma_loai_khach = khach_hang.ma_loai_khach
left join hop_dong
...
from dich_vu
join loai_dich_vu
on dich_vu.ma_loai_dich_vu = loai_dich_vu.ma_dich_vu
join hop_dong
...
from dich_vu
join loai_dich_vu
on loai_dich_vu.ma_dich_vu = dich_vu.ma_loai_dich_vu
join hop_dong
on hop_dong.ma_dich_vu = dich_vu.ma_dich_vu
...
from hop_dong
where hop_dong.ngay_lam_hop_dong = 2021)
...
--  bao gồm ma_hop_dong, ngay_lam_hop_dong, ngay_ket_thuc, tien_dat_coc,
...
where hdct.ma_dich_vu_di_kem = dvdk.ma_dich_vu_di_kem and
(lk.ten_loai_khach like '%Diamond%') and (kh.dia_chi like '%Vinh%' or kh.dia_chi like '%Quảng Ngãi%' );
...
-- sum so_luong ở dich_vu_di_kem), tien_dat_coc của tất cả các dịch vụ đã từng được khách 
...
on ldv.ma_dich_vu = dv.ma_loai_dich_vu
join hop_dong_chi_tiet hdct
on hdct.ma_hop_dong = hd.ma_hop_dong
join dich_vu_di_kem dvdk
on dvdk.ma_dich_vu_di_kem = hdct.ma_dich_vu_di_kem
...
alter table nhan_vien add trang_thai int default 0;
...
where ma_khach_hang in (
    select * from (
			    ...
			    having sum(dv.chi_phi + hdct.so_luong * dvdk.gia) > 10000000) as BT
             );
SET SQL_SAFE_UPDATES = 1;
...
(select * from 
               ...
               join hop_dong hd
               on hd.ma_khach_hang = kh.ma_khach_hang
               where year(hd.ngay_lam_hop_dong) < 2021) as BT);
SET SQL_SAFE_UPDATES = 1;
...
(select * from 
               ...
               from hop_dong hd
               join hop_dong_chi_tiet hdct
               on hdct.ma_dich_vu_di_kem = hd.ma_hop_dong
               join dich_vu_di_kem dvdk
               on dvdk.ma_dich_vu_di_kem = hdct.ma_dich_vu_di_kem 
               where hdct.so_luong > 10 ) as BT);
SET SQL_SAFE_UPDATES = 1;
...
union all 
select 
kh.ma_khach_hang , kh.ho_ten , kh.email , kh.so_dien_thoai , kh.ngay_sinh , kh.dia_chi
from khach_hang kh ;